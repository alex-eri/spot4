  ...
  function ($scope, $resource, $cookies, $location, $http) {
    ...
    function logoff() {
      if (ischilli)
        ...
        var url = new URL($cookies.get('linklogout'))
        ...
        $resource( url.href,
          ...
          { get: { method: 'JSONP', jsonpCallbackParam: 'var' } }).get().$promise.finally(
            onlogoff
          )
...
app.controller('Login', ['$window', '$resource', '$cookies', '$location', '$http', '$rootScope',
  function ($window, $resource, $cookies, $location, $http, $scope) {
    ...
    function onchillistatus(response) {
      ...
      if (response.redir.logoutURL) {
        var logoutURL = response.redir.logoutURL || $cookies.get('linklogout');
        var url = new URL(logoutURL.replace('/jsonp/', '/'))
        url.searchParams.set('target', '')
        logoutURL = url.href
        $cookies.put('linklogout', logoutURL)
    ...
    function onmikrotikstatus(response) {
      ...
      if (response.redir.logoutURL) {
        ...
        var logoutURL = response.redir.logoutURL
        var url = new URL(logoutURL.replace('/jsonp/', '/'))
        url.searchParams.set('target', '')
        logoutURL = url.href
        $cookies.put('linklogout', logoutURL)
...
app.controller('Status', ['$rootScope', '$resource', '$cookies', '$location', '$window',
  function ($scope, $resource, $cookies, $location, $window) {
    var ischilli = $cookies.get('uamip');
    function onlogin(response) {
      if ($cookies.get('linklogin').endsWith('uam/voucher/login')) {
      ...
      if ($location.$$search.online == 'yes') {
        console.log(response)
        if (response.redir.redirectionURL) {
          var url = new URL(response.redir.redirectionURL)
          url.searchParams.set(target='')
          console.log(response.redir.redirectionURL)
    ...
    $scope.logoutURL = $cookies.get('linklogout')
